...
class CarbonCopyExtractor:
    ...
    def extract(self, pdf_path: Path) -> CarbonCopyResult:
        ...
        with pdfplumber.open(str(pdf_path)) as pdf:
            ...
            for statement, pages in statement_pages.items():
                ...
                wide_df = self._build_wide_table(statement, pdf, pages)
                ...
                block = StatementBlock(statement, header, wide_df)
    ...
    def _build_wide_table(
        ...
        period_columns = reorder_period_columns(period_labels)
        ...
        for order_index, row in enumerate(parsed_rows, start=1):
            ...
            value_map = {
                period_labels[idx]: row["values"][idx]
                for idx in range(len(period_labels))
            }
            ordered_values = [value_map.get(col) for col in period_columns]
